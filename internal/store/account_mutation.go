...
// BindAccountMutationPresentation records immutable public coordinates before an add accepts input.
...
func validateAccountMutation(mutation AccountMutation) error {
	...
	} else if mutation.Kind != AccountMutationAdd && mutation.HasPresentationIdentity {
		return ErrAccountMutationState
	}
...
func validateAccountMutationReceipt(receipt AccountMutationReceipt) error {
	...
	if receipt.Kind == AccountMutationAdd {
		reservation := PendingAccountReservation{
	...
	if receipt.PublicationPending &&
		(receipt.Terminal != AccountMutationCommitted ||
			receipt.Kind != AccountMutationAdd ||
...
func (kind AccountMutationKind) valid() bool {
	switch kind {
	case AccountMutationAdd, AccountMutationRelogin:
		return true
	default:
		return false
	}
