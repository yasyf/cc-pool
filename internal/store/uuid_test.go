...
// TestGetAccountByUUIDDuplicatePicksLowestID pins deterministic resolution of
// duplicate uuids (the schema allows them; doctor flags them): always the
// lowest id, stable across calls — never an arbitrary row that could flap
// between sync resolutions.
