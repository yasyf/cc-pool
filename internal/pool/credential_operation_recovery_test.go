...
func TestExpiredMoveRecoveryIsDeterministicAtEveryPartialState(t *testing.T) {
	for _, tc := range []struct {
		name       string
		stage      func(t *testing.T, fake *credstest.Fake, account store.Account)
		wantStatus store.CredentialTerminalStatus
		wantResult store.CredentialResultCategory
		wantKC     bool
		wantFile   bool
	}{
		{
			name:       "source only safely rolls back",
			stage:      func(_ *testing.T, _ *credstest.Fake, _ store.Account) {},
			wantStatus: store.CredentialTerminalFailed, wantResult: store.CredentialResultFailed,
			wantKC: true,
		},
		{
			name: "matching source and target roll forward",
			stage: func(t *testing.T, _ *credstest.Fake, account store.Account) {
				writeRecoveryFileCredential(t, account, moveCred())
			},
			wantStatus: store.CredentialTerminalSucceeded,
			wantResult: store.CredentialResultMoved,
			wantFile:   true,
		},
		{
			name: "target only completes the move",
			stage: func(t *testing.T, fake *credstest.Fake, account store.Account) {
				writeRecoveryFileCredential(t, account, moveCred())
		...
		{
			name: "divergent source and target quarantine",
			stage: func(t *testing.T, _ *credstest.Fake, account store.Account) {
				writeRecoveryFileCredential(t, account, datedCred("divergent", time.Hour))
			},
			wantStatus: store.CredentialTerminalQuarantined,
			wantResult: store.CredentialResultDiverged,
			wantKC:     true,
			wantFile:   true,
		},
		{
			name: "unrelated target replacement quarantines",
			stage: func(t *testing.T, fake *credstest.Fake, account store.Account) {
				writeRecoveryFileCredential(t, account, datedCred("unrelated-target", time.Hour))
				fake.Remove(account.KeychainService, account.KeychainAccount)
			},
			wantStatus: store.CredentialTerminalQuarantined,
			wantResult: store.CredentialResultAmbiguous,
			wantFile:   true,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			st := openTestStore(t)
			account := persistTestAccount(t, st, store.Account{
				ID: 1, ConfigDir: t.TempDir(), KeychainService: "service-move-recovery", KeychainAccount: "account-move-recovery",
			})
			credentials := credstest.NewFake()
			credentials.Put(account.KeychainService, account.KeychainAccount, moveCred())
			manager := credentialRecoveryManager(t, st, credentials, "move-owner")
			...
			operation := beginRetiredCredentialOperation(
				...
				store.CredentialTargetFile,
				credentialIntentDigest(store.CredentialOperationMove, creds.SourceFile.String()),
				before,
			)
			tc.stage(t, credentials, account)
			...
			if receipt.TerminalStatus != tc.wantStatus || receipt.Result != tc.wantResult {
				t.Fatalf("receipt = %+v, want status=%s result=%q", receipt, tc.wantStatus, tc.wantResult)
			}
			if _, ok := credentials.Get(account.KeychainService, account.KeychainAccount); ok != tc.wantKC {
				t.Errorf("keychain present = %v, want %v", ok, tc.wantKC)
			}
			if got := fileCredentialExistsForTest(account.ConfigDir); got != tc.wantFile {
				t.Errorf("file present = %v, want %v", got, tc.wantFile)
			}
		})
	}
}
...
func TestExpiredCompensationRecoveryDoesNotDeleteDriftedState(t *testing.T) {
	st := openTestStore(t)
	account := persistTestAccount(t, st, store.Account{
		ID: 1, ConfigDir: t.TempDir(), KeychainService: "service-compensate-drift", KeychainAccount: "account-compensate-drift",
	})
	credentials := credstest.NewFake()
	written := datedCred("written-before-drift", time.Hour)
	credentials.Put(account.KeychainService, account.KeychainAccount, written)
	writeRecoveryFileCredential(t, account, written)
	manager := credentialRecoveryManager(t, st, credentials, "compensate-drift-owner")
	before, err := manager.credentialObservation(t.Context(), account)
	if err != nil {
		t.Fatal(err)
	}
	...
	credentials.Remove(account.KeychainService, account.KeychainAccount)
	drifted := datedCred("unrelated-later-login", 2*time.Hour)
	writeRecoveryFileCredential(t, account, drifted)
	if err := recoverExpiredCredentialOperation(t, manager, operation); err != nil {
		t.Fatalf("recover drifted compensation: %v", err)
	}
	onDisk, err := readFileCredentialForTest(account.ConfigDir)
	if err != nil {
		t.Fatal(err)
	}
	if onDisk.ClaudeAiOauth.AccessToken != drifted.ClaudeAiOauth.AccessToken {
		t.Fatalf("drifted credential was replaced: got %q want %q", onDisk.ClaudeAiOauth.AccessToken, drifted.ClaudeAiOauth.AccessToken)
	}
	if got := credentials.DeletedServices(); len(got) != 0 {
		t.Fatalf("drifted recovery Keychain deletes = %v, want none", got)
	}
	receipt, err := st.CredentialOperationReceipt(operation.Token)
	if err != nil {
		t.Fatal(err)
	...
	if receipt.TerminalStatus != store.CredentialTerminalQuarantined ||
		receipt.Result != store.CredentialResultChangedUnderfoot {
		t.Fatalf("drifted compensation receipt = %+v", receipt)
	}
}
