...
func TestReleaseValidatesDistributedApplication(t *testing.T) {
...
func TestReleasePublishesFormulaAndRetiresCaskAtomically(t *testing.T) {
	release := readReleaseContract(t, ".github", "workflows", "release.yml")
	...
	if got := strings.Count(release, ".github/actions/publish@"); got != 1 {
		t.Fatalf("tap publication calls = %d, want one atomic formula-and-retirement call", got)
	}
	publishJob := strings.Index(release, "\n  publish-tap:")
	formula := strings.Index(release, "Render the formula into the atomic tap transaction")
	publish := strings.Index(release, "Publish formula and retire the cask in one tap commit")
	if publishJob < 0 || formula < publishJob || publish < formula {
		t.Fatal("formula and exact cask retirement are not published after the verified stack")
	}
	transaction := release[publish:]
	for _, required := range []string{
		...
		if !strings.Contains(transaction, required) {
			t.Fatalf("atomic tap retirement is missing %q", required)
		}
	}
	if strings.Contains(release, "Render the cask into the atomic tap transaction") ||
		strings.Contains(release, "output: Casks/cc-pool-status.rb") {
		t.Fatal("release still renders the retired status cask")
	}
}
