...
// silently; cc-pool logs it once per episode here (proc owns no callback for
// the spare), reset by the next reachable Probe.
func (p *holderPolicy) PeerAlive() bool {
	alive := p.s.peerAliveOn(p.s.holderSocket)
	if alive && !p.loggedWedgedAlive {
...
// WaitGone's last probe and now) is translated to proc.ErrHolderUnavailable so
...
func (p *holderPolicy) Kill() (int, error) {
	...
	if errors.Is(err, mountd.ErrUnreachable) {
		return 0, fmt.Errorf("%w: %w", proc.ErrHolderUnavailable, err)
	}
	if err != nil {
...
//   - ChildDied (Alive==false only): force-unmount the dead holder's orphaned
//     carcasses before the respawn (the kill-9 whole-machine hazard), and stash
//     the carry / row-dir snapshot for the Respawned remount.
...
func (p *holderPolicy) Reconcile(ctx context.Context, ev proc.ReconcileEvent) {
	switch ev.Kind {
	case proc.ChildDied:
		// proc only fires ChildDied with Alive==false (contract 1: an alive-but-
		// wedged holder is spared), so this is always the genuine-death path. It
		// fires BEFORE the respawn, while carriedBases still holds the dead holder's
		// registry — snapshot it (and the row-dir set) here for the Respawned
		// reconcile, which runs after verifySpawned's refresh wiped the cache.
...
// surface proc itself does not own. Wired to proc.Supervisor.OnSpawnError.
func (p *holderPolicy) onSpawnError(err error) {
	p.s.holder.recordSpawnError(err.Error())
	if err.Error() != p.lastSpawnErr {
		p.lastSpawnErr = err.Error()
		p.s.log.Printf("spawn mount holder failed: %v", err)
	}
}
