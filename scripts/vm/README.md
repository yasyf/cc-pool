...
| --- | --- | --- |
| `verify-signed-topology` | Developer ID host/File Provider profiles and successful `push` | Exact host, File Provider, and widget identities; same-Team universal bundled fuse-t with reviewed license and library validation intact; one host/FP App Group; one host Mach-O; native-child dispatch; broker/holder sockets; unchanged protected-filesystem TCC rows; zero daemon TCC rows or App Group capability. |
